...
| `Character` | **0x4c** | TRACED — `operator_new(0x4c)` in `Dta_ParseChar` (0x421e70) → `Character_Read` (0x4047a0) → `World.characters@0xc0` (`Character**`) | DA `ichr_data`: `magic@0`, `unk1@4`, `name[0x10]@8`, `flags@0x18` (FRIENDLY=1/ENEMY=2/IS_WEAPON=4; **behaviour in the high word** `flags>>16` = `CharBehavior`: HARD=1/MEDIUM=2/RANDOM=3/STATIONARY=4/ANIMATED=8 — the short at `Char+0x1a` that `GameView::Tick` switches on for enemy AI), `frames[24]@0x1c` (anim ids; `Game_DrawEntities` draws `frames[0x10]`) |
