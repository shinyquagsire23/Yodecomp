...
| `Character` | **0x4c** | TRACED — `operator_new(0x4c)` in `Dta_ParseChar` (0x421e70) → `Character_Read` (0x4047a0) → `World.characters@0xc0` (`Character**`) | DA `ichr_data`: `magic@0`, `unk1@4`, `name[0x10]@8`, `flags@0x18` (FRIENDLY=1/ENEMY=2/IS_WEAPON=4; high word `flags>>16` = DA `ICHR_BEHAVIOR`/`CharBehavior` HARD=1/MEDIUM=2/RANDOM=3/STATIONARY=4/ANIMATED=8). **`moveType@0x36`** (a short in the frames region; `GameView::Tick` switches on it for enemy movement AI — cases 1/2/3/4/6/7/8/9/10, NOT the flags value; see game-logic.md), **`damage@0x3e`** (melee damage the entity deals via AddHealth), `framesA[13]@0x1c`/`framesB[3]@0x38`/`framesC[6]@0x40` (sprite anim ids; `Game_DrawEntities` draws ~`framesB[2]`=idx16), with two metadata slots carved out of the frames blob: **`moveType@0x36`** (`CharMoveType` enum — the enemy-AI selector `GameView::Tick` switches on) and **`damage@0x3e`** (melee damage, `AddHealth(-damage)`) |
